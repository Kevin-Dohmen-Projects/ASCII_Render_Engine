...
public class Poly3DWireframeBackfaceCullingRenderer : IPoly3DRenderer
{
    public void Render(ScreenBuffer buffer, int frame, double runTime, Poly3D obj)
    {
        ...
        {
            transformedPoly.Vertices[i].Position = Camera.Camera.TranslateToWorldSpace(Vertices[i].Position);
        }
        ...
        // backface culling
        if (Vec3.Dot(normal, (transformedPoly.Vertices[0].Position - new Vec3(0))) > 0)
            return;
        ...
        // Draw the wireframe transformedPoly.Vertices.Length
